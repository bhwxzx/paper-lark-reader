...
本项目的所有本地 Python 解析脚本，严重依赖 `paper_reader` 这个 Conda 虚拟环境中的第三方库（如 `pdfplumber`）。

为了防止环境错位导致模块找不到，你必须**绝对遵守**以下执行规范：
- **绝对禁止**直接使用 `python scripts/...` 或 `python3 scripts/...` 来运行脚本。
- **唯一合法的执行格式**：每一次调用本地 Python 脚本时，都必须使用 Conda 执行器进行包裹。格式固定为：
  `conda run -n paper_reader python scripts/<具体脚本名.py> <参数>`
